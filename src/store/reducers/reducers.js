...
export default function searchReducer(state = initialState, action) {
    switch (action.type) {
        ...
        case PAGINATOR:
            return {
                ...state,
                paginator: state.paginator + action.data,
            }
...
const priceReducer = (state, action) => {
    const newState = applyFilters({
        ...
        filters: {
            ...state.filters,
            price: { ...state.filters.price, ...action.data },
        },
    })
...
const getAirlines = (data) => [
    ...new Set(data.map((el) => el.flight.carrier.caption)),
]
...
const getPrices = (data) => {
    const res = data.map(
        (el) => el.flight.price.passengerPrices[0].singlePassengerTotal.amount
    )
...
const airlineFilter = (data, filters) => {
    return data.filter(
        (el) => filters.airline.indexOf(el.flight.carrier.caption) !== -1
    )
}
...
const sortByOrder = (data, filters) => {
    switch (filters.sortOrder) {
        ...
        case 'по времени в пути':
            return data.sort((a, b) => {
                const durA = a.flight.legs.reduce(
                    (prev, el) => prev + el.duration,
                    0
                )
                const durB = b.flight.legs.reduce(
                    (prev, el) => prev + el.duration,
                    0
                )
                return durA - durB
